...
export default class PoFilesCreator implements IFilesCreator {
  private supportedExtension = 'po';
  ...
  public save(dataToSave: ITranslationsData, path: string, filename: string): void {
    ...
    dataToSave.result.forEach((data: any) => this.createFolderAndSave(data.content, path, `plugin-${data.lang}`));
  }
