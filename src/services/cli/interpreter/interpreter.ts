...
export default class Interpreter {
    ...
    private configureCli(overwriteShadowArgs: undefined | string[]): Arguments {
        // console.log('overwriteShadowArgs', overwriteShadowArgs);
        // console.log('this.shadowArgs', this.shadowArgs)
        let parser = yargs
        ...
        if (this.shadowArgs || overwriteShadowArgs) {
            parser = parser.fail((msg, err) => { 
                console.log(`${msg ? `Failure message: ${msg} ` : ''}${err ? `Error: ${err}` : ''}`); 
                process.exit(1); })
        }
    ...
    async interpret(overwriteShadowArgs?: string[] | undefined) {
        const args: Arguments = this.configureCli(overwriteShadowArgs);
        args._[0] === 'init'
            ? await generateConfigFile(
                this.logger,
                this.inEnvStorage,
                this.googleAuth, args, this.getProperStorage[args['config-format']])
